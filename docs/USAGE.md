...
# Optional: extra PDF fallback parser. Note that it pulls in PyMuPDF, which is
# licensed AGPL-3.0 (or a commercial Artifex licence) — the default install
...
**Duplicate documents across folders** — if the exact same file is placed under two different `raw/` subfolders (e.g. `raw/erp12/policy.md` and `raw/legal/policy.md`), doqqy detects it by comparing each doc's processed-markdown `content_hash` (byte-identical bodies only — never fuzzy matching) and embeds it **once**. The alphabetically-first `doc_id` path becomes the canonical document (deterministic across machines, checkouts, and file copies, unlike `mtime`/`ctime`); the other is recorded in the manifest as an alias with no chunks of its own. Search results intentionally display the canonical document's path only (avoiding duplicate hits), while the canonical inherits the **union** of both folders' tags, so `--tag erp12` and `--tag legal` both find it. If the canonical document is deleted, `doqqy sync` automatically promotes and re-embeds the surviving duplicate(s) in that same run without needing a second sync. Run `doqqy info` to see duplicate groups.

## 3. CLI reference

### `doqqy ingest`

Convert `raw/` files (`.md`, `.markdown`, `.txt`, `.html`, `.htm`, `.pdf`, `.docx`, `.pptx`, `.xml`, `.xlsx`, `.csv`) to canonical markdown in `processed/`.

```powershell
doqqy ingest                     # all of raw/
doqqy ingest --ocr               # enable OCR fallback for scanned or image-only PDFs (slower)
...
doqqy watch
doqqy watch --debounce 3.0  # batch for at most 3 seconds from the first change
```
...
**A PDF produced garbage / empty output** — probably a scanned (image-only) PDF. Install OCR extras via `pip install -e ".[ocr]"` and run `doqqy ingest --ocr` to enable OCR fallback via Docling's OCR pipeline (requires EasyOCR / Tesseract bindings). The chain is docling → docling-ocr (with `--ocr`) → pymupdf4llm. That last step is an optional extra (`pip install -e ".[pdf-fallback]"`) and is skipped silently when it isn't installed, since PyMuPDF is AGPL-3.0 licensed.
