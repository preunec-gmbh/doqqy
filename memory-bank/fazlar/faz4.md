...
Faz 3 sonunda elimizde `.doqqy/topics.yaml` var: her section için `explicit_related` ve `might_be_related` listeleri. Ama bu veri yalnızca `INDEX.md`'de okunabilir formatta — Obsidian **graph view boş**, çünkü markdown dosyaları içinde hiç `[[wikilink]]` yok.

Faz 4 bu boşluğu dolduruyor:

- `processed/*.md` dosyalarının her biri `.doqqy/topics.yaml`'daki ilişkilerine göre güncellenir.
...
1. `.doqqy/topics.yaml` oku → tüm section bağlantılarını bir lookup dict'e dönüştür: `{file: {explicit: [...], tematic: [...]}}`
...
```
doqqy inject          # .doqqy/topics.yaml → processed/*.md içine [[link]] enjekte et
doqqy inject --dry-run  # Neyin enjekte edileceğini göster, dosyaları değiştirme
doqqy inject --topics path/to/.doqqy/topics.yaml  # Farklı topics dosyası
```
...
- [x] Graph view'da dosyalar arası kenar (edge) görünüyor mu?
- [x] `INDEX.md`'den dosyalara tıklama navigasyonu çalışıyor mu?
- [x] Explicit linkler (`📌`) ve tematik linkler (`🔗`) `INDEX.md`'de ayrı görünüyor mu?
- [x] Hiç `[[...]]` içermeyen dosyalar graph'ta yalnız düğüm olarak mı görünüyor (beklenen)?
- [x] Çok bağlantılı dosyalar (hub nodes) tespit edilebiliyor mu?

## 8. Dosya Değişiklikleri

| Dosya | Değişiklik |
|---|---|
| `src/doqqy/wikilink_inject.py` | **YENİ** — `.doqqy/topics.yaml` → `processed/*.md` enjeksiyon |
...
- [x] `src/doqqy/wikilink_inject.py` oluştur
  - [x] `.doqqy/topics.yaml` okuma + file→links lookup dict üretimi
  - [x] Marker blok silme (idempotent temizlik)
  - [x] Explicit + tematik linkleri dedupe + sırala
  - [x] Dosya sonuna blok enjeksiyonu
  - [x] `--dry-run` modu
  - [x] `InjectionResult` (güncellenen dosya sayısı, toplam link sayısı)
- [x] `src/doqqy/cli.py` — `doqqy inject` komutu ekle (`--dry-run`, `--topics`)
- [x] `processed/` klasörünü Obsidian'da vault olarak aç
- [x] Graph view doğrulaması (edge'ler görünüyor mu)
- [x] `INDEX.md`'den navigasyon testi
- [x] Smoke test: `doqqy inject --dry-run` önce, sonra gerçek çalıştır
...
| `.doqqy/topics.yaml`'da section ID → dosya adı eşlemesi bozulur | Düşük | ID formatı `FILENAME_section-slug` — `_` ile split, ilk parça dosya adı |
