...
raw/<her şey>
   │
   ▼ doqqy ingest        (router.py uzantıya göre delegasyon)
...
### 4.1 `config.py`
Tüm yollar buradan türetilir; her modül `from doqqy.config import ...` ile alır. Önemli sabitler:
...
`detect_device()` — `DOQQY_DEVICE` env varsa onu, yoksa `torch.cuda.is_available()` sonucuna göre `cuda`/`cpu` döndürür. Torch import maliyetinden kaçınmak için lazy.
...
`python -m doqqy <cmd>` veya `pip install -e .` sonrası `doqqy <cmd>` çalışır.
...
**Bge-m3 modeli:** ilk `doqqy embed` çağrısında HuggingFace'ten ~2 GB iner (tek seferlik, cache'lenir).
...
doqqy ingest -n 5
doqqy chunk
doqqy embed                       # ilk çağrı yavaş (model indirme)
...
doqqy chunk
doqqy embed
doqqy query "PayTR iade akışı"
doqqy query "Cari kart ekleme stored procedure" --full
```

## 8. Bilinen Sınırlamalar

- **Inkremental update yok** — `doqqy ingest` her seferinde tam rebuild yapar (memory-bank kararı).
