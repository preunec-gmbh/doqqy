...
- `ingest/` — format-specific parsers dispatched by extension in `router.py`: `.md`/`.txt` → `md_ingest.py`; `.pdf` → docling with pymupdf4llm fallback; `.docx` → pandoc with mammoth fallback; `.xml` → `xml_ingest.py` (etree). `base.py` defines `Document`, `content_hash`, and `base_metadata` (which derives `tags` from the folder structure under `raw/`: `raw/erp12/faturalama/x.md` → `tags: ["erp12", "faturalama"]`). Output frontmatter records `source`, `type`, `content_hash`, `parser`.
